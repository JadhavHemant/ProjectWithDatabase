...
const Framework = () => {
  ...
  useEffect(() => {
    ...
    //    .then((resp) => setUser(resp.data))
        ...
        const response = await axios.get(API.GROUPNAME_GOVERNANCE_API);
...
//   };
  ...
  const labels = groupName.map((item) => item.groupname_name);
  const data = groupName.map((item) => item.groupname_count);

  const chartData = {
    labels,
    datasets: [
      {
        label: 'Group Name Distribution',
  ...
  const options = {
    ...
    onClick: (_, elements) => {
      if (elements.length > 0) {
        ...
        handleGroupNameClick(clickedIndex);
      }
    },
  };

  const handleGroupNameClick = (index) => {
    const clickedGroupName = labels[index];
    console.log('Bar clicked! Navigating to:', `/thrustareagraph/${encodeURIComponent(clickedGroupName)}`);
   const newTab= window.open(`/thrustareagraph/${encodeURIComponent(clickedGroupName)}`, '_blank');
   newTab.focus();
  }
